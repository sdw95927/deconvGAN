...
def normalize(mat, quantile_low=0, quantile_high=1):
    ...
    mat_normalized = (mat - np.quantile(mat, quantile_low))/(np.quantile(mat, quantile_high) - np.quantile(mat, quantile_low))
...
def channel_deconvolution (img_RGB, staining_type, plot_image=False, to_normalize=True, 
                           stain_OD=None, channels=("Hematoxylin", "Eosin", "Background")):
    ...
    Args:
        ...
        staining_type: Dyes used to stain the image; choose one from ("HDB", "HRB", "HDR", "HEB").
                       To costomize stain, use "costom" and define stain_OD (list) and channels (list).
    ...
    elif staining_type == "costom":
        stain_OD = np.asarray(stain_OD)
    else:
        raise Exception("Staining type not defined. Choose one from the following: HDB, HRB, HDR, HEB, costome.")
